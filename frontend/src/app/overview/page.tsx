...
/* ---------------- key helpers ---------------- */
type NT = Exclude<RedundancyLevel, "global">;
type KeyStr = string;
...
function buildIndex(tree: DhcpLightTreeDto) {
  ...
  const addressBlockByAddress = new Map<string, KeyStr>();
  const addressBlockByName = new Map<string, KeyStr>();
  const subnetByAddress = new Map<string, KeyStr>();
  const subnetByName = new Map<string, KeyStr>();
  const rangeByTuple = new Map<string, KeyStr>();
  const fixedByTuple = new Map<string, KeyStr>();
  ...
  const indexSel = (sel: TreeSelection, parentKey: KeyStr) => {
    ...
    switch (sel.type) {
      ...
      case "addressBlock": {
        ...
        const name = s(rd(sel.object, "name"));
        if (addr) {
          addressBlockByAddress.set(addr, key);
          if (cidr) addressBlockByAddress.set(`${addr}/${cidr}`, key);
        }
  ...
  const walkAddressBlock = (ab: LightAddressBlockDto, parentKey: KeyStr) => {
    ...
    for (const child of abChildren(ab)) walkAddressBlock(child, abKey);
    for (const sn of abSubnets(ab)) walkSubnet(sn, abKey);
  };
  ...
  for (const ip of (tree.ipSpaces ?? []) as LightIpSpaceDto[]) {
    ...
    for (const ab of ipSpaceAddressBlocks(ip)) walkAddressBlock(ab, ipKey);
    for (const sn of ipSpaceSubnets(ip)) walkSubnet(sn, ipKey);
  }
...
export default function OverviewPage() {
  ...
  const [tab, setTab] = useState<"tree" | "overview" | "groups" | "redundancies">(
    "tree"
  );
  ...
  const [hashChanged, setHashChanged] = useState<boolean>(false);

  useEffect(() => {
    const stored = localStorage.getItem("hashChanged");
    if (stored === "true") {
      setHashChanged(true);
      setLoading(false);
      return;
  ...
  const onImportComplete = async () => {
    ...
    localStorage.removeItem("hashChanged");
  ...
  if (hashChanged || !tree || !tree.ipSpaces || tree.ipSpaces.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh]">
        <p className="text-gray-400 text-xl mb-6">
          {hashChanged
            ? "No data available – please start import"
            : "No data available"}
        </p>
