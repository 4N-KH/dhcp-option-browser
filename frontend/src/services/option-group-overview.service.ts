...
export async function fetchOptionGroupOverview(): Promise<
  OptionGroupOverviewDto[]
> {
  const res = await fetch(`${API_BASE_URL}/api/option-group-overview`, {
    credentials: "include",
  });
...
export async function fetchOptionGroupOccurrences(
  groupId: number,
): Promise<OptionGroupOccurrenceDto[]> {
  const res = await fetch(
    `${API_BASE_URL}/api/option-group-overview/${groupId}/objects`,
    { credentials: "include" },
  );
...
export async function fetchOptionGroupOptions(
  groupId: number,
): Promise<OptionInGroupDto[]> {
  const res = await fetch(
    `${API_BASE_URL}/api/option-group-overview/${groupId}/options`,
    { credentials: "include" },
  );
